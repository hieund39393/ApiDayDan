using Authentication.Application.Commands.DM_BieuGia_CapNgamCommand;
...
namespace Authentication.API.Controllers
{
    ...
    public class BieuGiaCapNgamController : ControllerBase
    {
        private readonly IDM_BieuGia_CapNgamQuery _bieuGiaQuery; //kế thừa interface
        private readonly IMediator _mediator; //kế thừa để sử dụng command

        public BieuGiaCapNgamController(IMediator mediator, IDM_BieuGia_CapNgamQuery bieuGiaQuery)
        {
            _mediator = mediator;
        ...
        public async Task<IActionResult> Create([FromBody] Create_DMBieuGia_CapNgamCommand command)
        {
            var user = await _mediator.Send(command);
            return Ok(new ApiSuccessResult<bool>(data: user, message: string.Format(Resources.MSG_CREATE_SUCCESS, "Biểu giá cáp ngầm")));
        }
        ...
        [HttpPut]
        [ProducesResponseType(typeof(ApiSuccessResult<bool>), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> Update([FromBody] Update_DM_BieuGia_CapNgam_CapNgamCommand command)
        {
            var user = await _mediator.Send(command);
            return Ok(new ApiSuccessResult<bool>(data: user, message: string.Format(Resources.MSG_UPDATE_SUCCESS, "biểu giá cáp ngầm")));
        }
        ...
        {
            var data = await _mediator.Send(new Delete_DM_BieuGia_CapNgamCommand(id));
            return Ok(new ApiSuccessResult<bool>(data: data, message: string.Format(Resources.MSG_DELETE_SUCCESS, "biểu giá cáp ngầm")));
        }
    }
}
