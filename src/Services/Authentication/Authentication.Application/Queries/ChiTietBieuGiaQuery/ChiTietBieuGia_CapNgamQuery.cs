...
namespace Authentication.Application.Queries.ChiTietBieuGia_CapNgamQuery
{

    public interface IChiTietBieuGia_CapNgamQuery // tạo interface (Quy tắc : có chữ I ở đầu để biết nó là interface)
    {
    ...
    public class ChiTietBieuGia_CapNgamQuery : BaseExtendEntities, IChiTietBieuGia_CapNgamQuery // kế thừa interface vừa tạo
    {
        ...
        public ChiTietBieuGia_CapNgamQuery(IUnitOfWork unitOfWork, IExOneHttpClientFactory httpClientFactory)
        {
        ...
        {
            return await _unitOfWork.DM_BieuGia_CapNgamRepository.GetQuery(x => x.idLoaiBieuGia == loaiBieuGia).Select(x => new SelectItem
            {
                Name = x.TenBieuGia,
                Value = x.Id.ToString()
        ...
        public async Task<List<GetDonGiaResponse>> GetDonGia(GetDonGiaRequest request)
        {
            ...
            switch (request.Nguon)
            {
                ...
                case 2:
                    result = await _unitOfWork.DonGiaVatLieu_CapNgamRepository.GetQuery()
