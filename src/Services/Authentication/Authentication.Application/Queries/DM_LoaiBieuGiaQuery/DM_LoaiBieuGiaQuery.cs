...
namespace Authentication.Application.Queries.DM_LoaiBieuGiaQuery
{
    ...
    public class DM_LoaiBieuGiaQuery : IDM_LoaiBieuGiaQuery // kế thừa interface vừa tạo
    {
        ...
        {
            var query = await _unitOfWork.DM_LoaiBieuGiaRepository.GetQuery().Include(x => x.DM_KhuVuc).Select(x => new SelectItem
            {
        ...
        {
            var query = await _unitOfWork.DM_LoaiBieuGiaRepository.GetQuery(x => x.IdKhuVuc == IdKhuVuc).Select(x => new SelectItem
            {
