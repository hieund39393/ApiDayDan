...
namespace Authentication.Application.Queries.DM_LoaiCap_CapNgamQuery
{
    public interface IDM_LoaiCap_CapNgamQuery // tạo interface (Quy tắc : có chữ I ở đầu để biết nó là interface)
    {
    ...
    public class DM_LoaiCap_CapNgamQuery : IDM_LoaiCap_CapNgamQuery // kế thừa interface vừa tạo
    {
        private readonly IUnitOfWork _unitOfWork; // khai báo 
        public DM_LoaiCap_CapNgamQuery(IUnitOfWork unitOfWork)
        {
        ...
        {
            var query = await _unitOfWork.DM_LoaiCap_CapNgamRepository.GetQuery().Select(x => new SelectItem
            {
                Name = x.TenLoaiCap,
                Value = x.Id.ToString(),
        ...
        {
            //Tạo câu query
            var query = _unitOfWork.DM_LoaiCap_CapNgamRepository.GetQuery(x =>
