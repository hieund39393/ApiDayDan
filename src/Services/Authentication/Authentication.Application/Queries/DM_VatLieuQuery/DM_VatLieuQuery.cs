...
namespace Authentication.Application.Queries.DM_VatLieuQuery
{
    public interface IDM_VatLieuQuery // tạo interface (Quy tắc : có chữ I ở đầu để biết nó là interface)
    {
    ...
    public class DM_VatLieuQuery : IDM_VatLieuQuery // kế thừa interface vừa tạo
    {
        private readonly IUnitOfWork _unitOfWork; // khai báo 
        public DM_VatLieuQuery(IUnitOfWork unitOfWork)
        {
        ...
        {
            var query = await _unitOfWork.DM_VatLieuRepository.GetQuery().Select(x => new SelectItem
            {
                Name = x.TenVatLieu,
                Value = x.Id.ToString(),
        ...
        {
            //Tạo câu query
            var query = _unitOfWork.DM_VatLieuRepository.GetQuery(x =>
